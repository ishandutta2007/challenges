...
The winning entry will be one with the lowest cost, obtained from the number of operators executed in the input-dependent data path, with a weight of 5 for unary operators - and ~ as well as for <<1, >>1, +1, -1; weight of 7 for all other operators,
shifts with other counts, or adds/subs of other constants (type casts and promotions are free).
By principle, that cost is unchanged by unrolling loops (if any), and independent of the input x. As a tie-breaker, the answer with the shortest code after removing whitespace and comments wins.
...
func subbytes_table(x int32) int {
	return stab[x]
}

// https://cryptojedi.org/peter/data/shmoocon-20150118.pdf
func subbytes_constant(x int32) int {
	r := stab[0]
	for i := int32(1); i < 256; i++ {
		v := subtle.ConstantTimeEq(i, x)
		r = subtle.ConstantTimeSelect(v, stab[i], r)
	}
	return r
}
