...
uvlong
max(uvlong a, uvlong b)
{
	return (a > b) ? a : b;
}

uvlong
fit1(uvlong X, uvlong Y, uvlong x, uvlong y)
{
	if (x == 0 || y == 0)
		return 0;
	return (X / x) * (Y / y);
...
uvlong
fit2(uvlong X, uvlong Y, uvlong x, uvlong y)
{
	return max(fit1(X, Y, x, y), fit1(X, Y, y, x));
}

uvlong
fit3s(uvlong X, uvlong Y, uvlong Z, uvlong x, uvlong y, uvlong z)
{
	if (x == 0 || y == 0 || z == 0)
		return 0;
	return (X / x) * (Y / y) * (Z / z);
...
uvlong
fit3(uvlong X, uvlong Y, uvlong Z, uvlong x, uvlong y, uvlong z)
{
	uvlong r;

	r = fit3s(X, Y, Z, x, y, z);
	r = max(r, fit3s(X, Y, Z, x, z, y));
	r = max(r, fit3s(X, Y, Z, y, x, z));
	r = max(r, fit3s(X, Y, Z, y, z, x));
	r = max(r, fit3s(X, Y, Z, z, x, y));
	r = max(r, fit3s(X, Y, Z, z, y, x));
	return r;
}
...
main(void)
{
	...
	assert(fit1(12345, 678910ull, 1112, 1314) == 5676);
	assert(fit1(5, 100, 6, 1) == 0);

	assert(fit2(25, 18, 6, 5) == 15);
	assert(fit2(12, 34, 5, 6) == 12);
	assert(fit2(12345, 678910ull, 1112, 1314) == 5676);
	...
	assert(fit3(123, 456, 789, 10, 11, 12) == 32604ull);
	assert(fit3(1234567ull, 89101112ull, 13141516ull, 171819ull, 202122ull, 232425ull) == 174648ull);
