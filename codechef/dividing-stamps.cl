...
; Now Mike wants to know if he was right. The next SCC meeting is tomorrow.
; Mike still has no answer.
;
...
(defun required (l)
  (if (= (reduce #'+ l) (triangular (length l))) "YES" "NO"))

(assert (string= (required '(7 4 1 1 2)) "YES"))
(assert (string= (required '(1 1 1 1 1)) "NO"))
