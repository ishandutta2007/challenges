...
Chef knows that the Nobel community is going to award the prize to that person whose research is different from others (ie. no other researcher should work on the same topic).
...
In total the CS field can be divided into M broad topics. Given the topics in which each of the N researchers are working on, in the form of an array A, and given that Chef can master any topic instantly, find whether he can win the prize.
That is, can the Chef choose to work on some topic which will guarantee that he will win the prize? Chef doesn't mind sharing the prize with others.
...
func main() {
	assert(winnable([]int{1, 2, 3, 4}, 4) == "NO")
	assert(winnable([]int{4, 2, 1, 1, 1}, 4) == "YES")
	assert(winnable([]int{4, 4, 4, 4}, 4) == "YES")
}
...
func winnable(a []int, n int) string {
	m := make(map[int]bool)
	for _, v := range a {
		m[v-1] = true
	}
