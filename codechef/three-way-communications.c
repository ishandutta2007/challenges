...
One is that even if two people cannot talk directly because they are out of range, if there is another transceiver that is close enough to both,
...
bool
allcom(int r, Point a, Point b, Point c)
{
	int r2;
	int d;
...
main(void)
{
	assert(allcom(1, (Point){0, 1}, (Point){0, 0}, (Point){1, 0}) == true);
	assert(allcom(2, (Point){0, 1}, (Point){0, 0}, (Point){1, 0}) == true);
	assert(allcom(2, (Point){0, 0}, (Point){0, 2}, (Point){2, 1}) == false);
